...
# Time-stamp: <Last changed 2025-05-29 22:03:51>
...
(use-package org
  ...
  (set-face-attribute 'org-column nil
                      :inherit 'default)
  (push '("w" "Work") org-capture-templates)
  (push '("wt" "Work TODO task"
          entry
          (file+headline "~/org/work.org" "Capture")
          (file "~/org/templates/tpl-todo.txt"))
        org-capture-templates)
  (push '("p" "Private life") org-capture-templates)
  (push '("pt" "Private TODO task"
          entry
          (file+headline "~/org/private.org" "Capture")
          (file "~/org/templates/tpl-todo.txt"))
        org-capture-templates)
  (push '("pj" "Journal entry"
          entry
          (file+olp+datetree "~/org/journal.org") "* %U - %^{Activity}")
          org-capture-templates)
  (push '("pb" "Add book to read"
          entry
          (file+headline "~/org/private.org" "Books to read")
          (file "~/org/templates/tpl-book.txt")
          :empty-lines-after 2)
        org-capture-templates)
  :bind
  ("\C-ca" . 'org-agenda)
...
#+end_src
** Mylife headers
Thanks to [Rainer König](https://www.udemy.com/course/getting-yourself-organized-with-org-mode)
#+begin_src
#+SEQ_TODO: REPEAT(r) NEXT(n@/!) TODO(t@/!) WAITING(w@/!) SOMEDAY(s@/!) PROJ(p) | DONE(d@) CANCELLED(c@)
#+STARTUP: nologrepeat logdrawer lognotereschedule
#+TAGS: PHONE(o) COMPUTER(c) SHOPPING(s) URGENT(u)
#+ARCHIVE: %s_archive::
#+COLUMNS: %58ITEM(Details) %7TODO %Effort(Estimation){:} %6CLOCKSUM(Actual)
